...
(defun f2 (beat notes dur)
  (if (> (random 1.0) 0.7)
      (playp *scale*
             dur 0 :baritone-sax (r-elt '(0 0 -12))
...
;;;  (format t "beat: ~4,2f~%" (float beat 1.0))
  (onbeat? 1 10
           (progn
             (setf *root* (r-elt (remove *root* '(-2 0 3 7 -4 9))))
             (setf *scale* (pc-scale *root* 'aeolian))
                (map nil (lambda (p) (play (random 3) :electric-piano-2 p 60 10 :channel 3))
...
;;;  (play 0 :piano (+ *root* 48) 70 1/5 :channel 7)
  ...
  (let ((v 50))
    ...
    (playp *scale* 10 3 :trombone 0
           `(36) v (* 20 dur) :channel 5))
